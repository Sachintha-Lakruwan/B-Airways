...
export async function GET(request: NextRequest) {
  ...
  try {
    const dep_flights: FlightSearchResult[] = await executeQuery(
      "CALL `reservation_flightSearch`(?, ?, ?)",
      [departure_airport, arrival_airport, departure_date]
    );
    const arr_flights: FlightSearchResult[] = await executeQuery(
      "CALL `reservation_flightSearch`(?, ?, ?)",
      [arrival_airport, departure_airport, arrival_date]
    );
